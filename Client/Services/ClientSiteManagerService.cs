...
namespace Client.Services
{
	public class ClientSiteManagerService
	{
		...
		public ClientSiteManagerService(GatewayService gateway)
		{
			...
			clientSiteManager.OnDoesGameNameExistReceived += (user, model) => { if (OnDoesGameNameExistReceived != null) OnDoesGameNameExistReceived(user, model); };
			clientSiteManager.OnDeveloperCreateGameReceived += (user, model) => { if (OnDeveloperCreateGameReceived != null) OnDeveloperCreateGameReceived(user, model); };
			clientSiteManager.OnDeveloperUpdateGameReceived += (user, model) => { if (OnDeveloperUpdateGameReceived != null) OnDeveloperUpdateGameReceived(user, model); };
