...
Client.PageHandler.prototype = {
	...
	resizeCanvas: function() {
		if (ss.Nullable.unbox(Type.cast(!ss.referenceEquals(this.$gameContext.canvas.attr('width'), ($(window)).width()), Boolean))) {
			this.$gameContext.canvas.attr('width', ($(window)).width() * 0.5);
		}
		if (ss.Nullable.unbox(Type.cast(!ss.referenceEquals(this.$gameContext.canvas.attr('height'), ($(window)).height()), Boolean))) {
			this.$gameContext.canvas.attr('height', ($(window)).height());
		}
		if (ss.Nullable.unbox(Type.cast(ss.isValue(this.$lastMainArea), Boolean))) {
