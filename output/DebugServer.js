...
DebugServer.DebugServer = function() {
	...
	queueManager.addChannel('Area.Debug2.GetGameSource.Request', function(sender, data) {
		fs.readFile('/usr/local/src/games/' + data.gameName + '/app.js', 'ascii', function(err, data2) {
			queueManager.sendMessage(sender, sender.gateway, 'Area.Debug.GetGameSource.Response', new Models.GameSourceResponseModel(data2));
		});
	});
};
